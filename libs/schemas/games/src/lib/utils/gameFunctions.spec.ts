...
describe('GameFunctions', () => {
  ...
  it('should generate a properly formatted game object', () => {
    ...
    const locKeys = Object.keys(game.locations[0]);
    const goodKeys =
  ...
  it('should have matching prints on the murder weapon and opposite on the other weapon', () => {
    const murdererPrints = characters.find(character => character.id = game.murderer)['odd']
      ? 'odd'
      : 'even';

    const otherPrints = characters.find(character => character.id = game.murderer)['odd']
      ? 'even'
      : 'odd';
